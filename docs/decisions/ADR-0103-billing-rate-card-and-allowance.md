...
**(1) 免费额度的分类 —— 本 ADR 的 D2 缺一维。** 那个参考实现把"免费"分成**四类**：

| 类别（**本 ADR 自己的命名**，不沿用对方的标识符） | 本 ADR 的 `FreeAllowance` 能表达吗 |
|---|---|
| **周期配额**（每隔一段时间重置的免费额度） | ✅ `reset_period_hours = 24` |
| **一次性试用**（只给一次，不再重置） | ✅ `reset_period_hours = 0` |
| **永久免费**（没有额度上限） | ❌ **本 ADR 强制要求 `quantity`**，表达不出"没有上限" |
| **周期信用**（周期性发放的**货币**额度，不是 token） | ❌ 本 ADR 是**单位量**模型，**没有货币/信用这一维** |

> **⚠️ 只取"四类"这个区分，不取它的名字与标识符。**
> 本节的上一版**逐字抄了对方的类型定义原文**，已删除 —— 用户规则：**只作灵感来源，不抄代码、不抄特有名词**。
> **四类这个"想法"可以用；它的"表达"（类型名、字段名、简称）不用。**

**⇒ 这正是用户说的"分离一个免费额度分类"。** D2 必须补**永久免费**与**信用**两种形态，
否则把一家"每月送 $5 信用"的供应商硬塞进"每月送 N token"的模型里，**比较会算错，而错得不可见**。

**(2) 与计费无关、但决定"这条路能不能走"的事实。** 那个参考实现在免费条目上还记了
**四类资格/合规事实**（**用我们自己的说法**）：**是否需要绑卡** · **是否需要手机号** ·
**是否允许商用**（三态：是 / 否 / 未知）· **核验时间**。
**这不是计费，是资格与合规** —— 但它们同样是**声明**，同样需要"何时核验过"的时间戳。
**⇒ 归供应商声明，不归 `BillingDeclaration`**（单一职责，别把两种事实混成一张表）。

**(3) 版本化目录本身就是声明。** 那个参考实现的目录带有**版本号 + 更新时间 + 条目表**
...
**只剩免费模型列表的优先级徽章**（一个界面文案键，与一个可拖拽、带数字徽章的有序列表组件）。
...
| 绑定要求 / 手机号要求 / 商用许可 / 核验时间（**我们的说法**） | ✅ **吸收**，归供应商声明，**不归计费** |
| 版本化目录 | ✅ 旁证（与 D2 一致） |
...
补**永久免费**与**信用**两种形态属接口变更，**是下一步的第一件**，此处先把它记成已采纳的决定，
避免它退化成又一个"声明了但没有入口"。
