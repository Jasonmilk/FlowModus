...
class TestPriorityCascade:
    ...
    def test_healthy_fast_lane_used(self):
        ...
        # Only fast-lane candidates returned
        assert len(result) == 2
    ...
    def test_degraded_excluded_without_rehab(self):
        ...
        # fast1 is DEGRADED and excluded, only fast2 remains
        assert len(result) == 1
    ...
    def test_terminal_excluded(self):
        ...
        # All fast lane excluded, fall back to backup
        assert len(result) == 1
    ...
    def test_all_groups_empty_returns_all(self):
        ...
        health_states = {"fast1": "TERMINAL", "fast2": "TERMINAL", "backup1": "TERMINAL", "other1": "HEALTHY"}
        instance_id = "test-instance"

        result = apply_priority_filter(candidates, priority_groups, health_states, instance_id)

        # Since all groups are excluded (no HEALTHY in any group), we return all original candidates
        assert len(result) == 4
        # other1 should be among results
        supplier_ids = {c.supplier_id for c in result}
